...
PowGate configuration is available in Phase 2. NGINX parses and validates all
directives, applies inheritance, loads the secret file, and registers the
pass-through access handler when protection is enabled. Challenge issuance,
request-time exemption matching, and cookie and proof verification arrive in
Phases 3 and 4.
...
Phase 2 parses and inherits exemption lists but does not yet apply them to
requests. Phase 3 will use this fixed evaluation order after skipping
...
bits set and logs its standard warning. Future request matching uses the
connection address after RealIP processing.
...
Future path matching is case-sensitive and uses NGINX's `r->uri`: the
...
This configuration is fully validated today, but the Phase 2 handler still
passes requests through. Phase 3 adds challenge responses and runtime
exemptions; Phase 4 adds proof and authentication-cookie verification.
