...
1. `pow_crypto`:
   ...
   - `pow_b64url_encode/decode` — strict alphabet, no padding, exact-length
     variants that fail unless decoded length matches expectation.
2. `pow_challenge`:
   - `pow_ip16_from_sockaddr_bytes(...)` helper contract defined here as
     pure input (16-byte canonical IP + prefix_len), masking function
     `pow_ip16_mask(ip16, prefix_len)` with hand-written IPv6 masking.
...
3. `pow_cookie`:
   ...
   - `pow_cookie_verify(secret, secret2_or_null, parsed, ip16, now,
     ...
     MAC (dual-secret, both always evaluated on failure), expiry,
     difficulty and plen floors (config values passed by the caller),
     per protocol.md §Verification.
4. Reference solver `tools/refsolve.py`: given
   ...
   second.
5. Canonical end-to-end test vector, generated by `refsolve` at difficulty
   8 and checked in as `tests/vectors/v1.json`: fixed secret, IP, plen,
   bucket → expected nonce (hex) → mined counter → proof digest (hex) →
   exact auth-cookie string. Consumed by the C unit tests now, by the JS
   solver unit test in Phase 4B, and by nothing generated at runtime —
   it exists to catch silent drift between implementations immediately.
6. Unit tests (table-driven) in `tests/unit/`:
   ...
   - b64url: padding chars rejected, `+`/`/` rejected, wrong decoded
     lengths rejected.
7. Fuzz harnesses in `tests/fuzz/` (libFuzzer + ASan/UBSan, clang):
...
```
Fuzzers run 10 minutes clean under ASan; unit tests 100% pass; `gcov`
shows every branch of both parsers covered by the unit tables.
